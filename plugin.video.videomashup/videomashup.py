...
def trailer(name):
	youtube_trailer_search = 'https://www.googleapis.com/youtube/v3/search?part=id,snippet&q=%s-Trailer-Legendado&maxResults=1&key=AIzaSyBGg2d1CkJnDOatB2uDJVWsFwQ06MMjRdM'
	ytpage = open_url(youtube_trailer_search % (urllib.quote_plus(name)))
	youtubeid = re.compile('"videoId": "(.+?)"').findall(ytpage)
	if not youtubeid:
		youtube_trailer_search = 'https://www.googleapis.com/youtube/v3/search?part=id,snippet&q=%s-Trailer&maxResults=1&key=AIzaSyBGg2d1CkJnDOatB2uDJVWsFwQ06MMjRdM'
		ytpage = open_url(youtube_trailer_search % (urllib.quote_plus(name)))
		youtubeid = re.compile('"videoId": "(.+?)"').findall(ytpage)
	try: url = 'plugin://plugin.video.youtube/play/?video_id=%s' % youtubeid[0]
	except: pass
	if url == None: return
...
def clean_safe(s):
    if not s: return ''
    try: s = clean1(clean2(clean3(smart_unicode(s))))
    except:
        if enable_debug: traceback.print_exc(file = sys.stdout)
    return s
...
class CCurrentList:
    ...
    def decodeUrl(self, url, url_type = 'rss'):
        ...
        for info_name_value in infos_names_values:
            ...
            if sep_index != -1: item.infos_dict[info_name_value[:sep_index]] = clean_safe(info_name_value[sep_index+1:])
    ...
    # Site specific info handling
        ...
        if info_name == 'title':
            try: info_value = clean_safe(info_value.replace('\r\n', '').replace('\n', '').replace('\t', ''))
            except: info_value = '...'
        ...
        for item_rule in self.rules:
            ...
            if item_rule.curr != '':
                ...
                for title in revid.findall(data):
                    tmp = CListItem()
                    if item_rule.skill.find('space') != -1: tmp.infos_dict['title'] = '  ' + clean_safe(title.lstrip().rstrip()) + ' (' + __language__(30106) +')  '
                    else: tmp.infos_dict['title'] = ' ' + clean_safe(title.lstrip().rstrip()) + ' (' + __language__(30106) +') '
...
class Main:
    ...
    def parseView(self, url):
        ...
        if ext == 'cfg' or ext == 'list': result = self.currentlist.loadLocal(url, lItem = lItem)
			...
			trailer(clean_safe(lItem.infos_dict['title']))
			return -2
    ...
    def addListItem(self, title, url, icon, totalItems, lItem):
        ...
        action = 'XBMC.RunPlugin(%s.trailer)' % u
        try: liz.addContextMenuItems([(__language__(30119), action)])
        except: pass
        for info_name, info_value in lItem.infos_dict.iteritems():
